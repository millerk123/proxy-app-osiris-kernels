...
// struct InitFields {
//   OffsetView<double****> d_e;
//   OffsetView<double****> d_b;
//   double **e;
//   double **b;
//   int lb, ub;
...
//   InitFields(OffsetView<double****> d_e_, OffsetView<double****> d_b_,
//              double **e_, double **b_, int lb_, int ub_) : d_e(d_e_), d_b(d_b_), e(e_),
//              b(b_), lb(lb_), ub(ub_) {}
...
//       for (int j = lb; j < ub; j++) {
//         for (int l = 0; l < 3; l++) {
//           d_e(e[i][l+3*(j+stride*k)] = ( ((double) rand()/RAND_MAX) - 0.5 ) * 2;
...
int main(void)
{
  ...
  Kokkos::Experimental::OffsetView<double****> d_e("d_e", {0, 2}, {-nguard, ncells+nguard-1}, {-nguard, ncells+nguard-1}, {0, ntils-1});
  Kokkos::Experimental::OffsetView<double****> d_b("d_b", {0, 2}, {-nguard, ncells+nguard-1}, {-nguard, ncells+nguard-1}, {0, ntils-1});
  Kokkos::Experimental::OffsetView<double****> d_jay("d_jay", {0, 2}, {-nguard, ncells+nguard-1}, {-nguard, ncells+nguard-1}, {0, ntils-1});

  // Initialize the field arrays
  for (int i = 0; i < ntils; i++) {
    // Create unmanaged view of existing field data
    // Kokkos::Experimental::OffsetView<double***, Kokkos::HostSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged> >
    // e_view (e[i]-offset, {0, 2}, {-nguard, ncells+nguard-1}, {-nguard, ncells+nguard-1});
    // Kokkos::Experimental::OffsetView<double***, Kokkos::HostSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged> >
    // e_view (e_cust, {0, 2}, {-nguard, ncells+nguard-1}, {-nguard, ncells+nguard-1});
    Kokkos::View<double***, Kokkos::HostSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged> >
    e_view (e[i]-offset, 3, stride, stride);
    auto e_sub = Kokkos::subview (d_e, Kokkos::All(), Kokkos::All(), Kokkos::All(), i);
  }
  // Kokkos::parallel_for(ntils, InitFields(d_e, d_b, e, b, ub, lb));
